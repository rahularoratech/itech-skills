...
| --- | --- |
| A well-scoped feature you can settle in one sitting | [grill-me](https://github.com/rahularoratech/itech-skills/blob/main/docs/productivity/grill-me.md), or [grill-with-docs](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/grill-with-docs.md) when there is a codebase |
| A greenfield project, or a build spanning many sessions, with the route still unclear | `/wayfinder` |
| A thread where the deciding is already done | [to-spec](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/to-spec.md): skip straight past the map |
| A cleared wayfinder map | [to-spec](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/to-spec.md), then [to-tickets](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/to-tickets.md) and [implement](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/implement.md) |
| An existing session that has already grown too big | say "hand off to `/wayfinder`" ([handoff](https://github.com/rahularoratech/itech-skills/blob/main/docs/productivity/handoff.md) bridges into a map as well as out of one) |
...
The map and its tickets live on the repo's issue tracker, so wayfinder needs the tracker wiring that [setup-itech-skills](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/setup-itech-skills.md) lays down. That step writes a "Wayfinding operations" section describing how the map, its child tickets, blocking edges, and frontier queries are expressed for GitHub, GitLab, or local markdown. Wayfinder resolves that doc through the pointer in your `CLAUDE.md` / `AGENTS.md` rather than a fixed path; with no tracker configured at all it falls back to local markdown files.
...
| --- | --- | --- | --- |
| `grilling` | HITL | The default. The question can be settled by talking it through. | [grilling](https://github.com/rahularoratech/itech-skills/blob/main/docs/productivity/grilling.md) plus [domain-modeling](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/domain-modeling.md), in a fresh session |
| `prototype` | HITL | "How should this look" or "how should this behave": a question talking cannot settle. | [prototype](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/prototype.md), with the built artifact linked from the ticket as an asset |
| `research` | AFK | A fact outside the working directory is blocking a decision. | A [research](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/research.md) [subagent](https://www.aihero.dev/ai-coding-dictionary/subagent), fired at charting time and burned down in parallel on a `research/<name>` branch |
...
`wayfinder` is a **situational on-ramp**, not the default front door. The grill-led idea → ship chain is still where most work starts; wayfinder is what you climb onto when the idea is too big to hold in one session, and it merges back onto that chain at [to-spec](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/to-spec.md), because a cleared map hands off rather than builds.

Underneath, it is mostly other skills wearing wayfinder's scheduling: [grilling](https://github.com/rahularoratech/itech-skills/blob/main/docs/productivity/grilling.md) and [domain-modeling](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/domain-modeling.md) resolve the default ticket type, [prototype](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/prototype.md) resolves the tickets that talking cannot, and [research](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/research.md) runs as a subagent so its reading never lands in your session. [handoff](https://github.com/rahularoratech/itech-skills/blob/main/docs/productivity/handoff.md) is the bridge in and out: into a map from a conversation that outgrew itself, out of one when a side quest appears mid-session. For anything else, [ask-itech-skills](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/ask-itech-skills.md) routes over the whole set.
