...
For anything else (same harness, same directory, you are done [grilling](https://www.aihero.dev/ai-coding-dictionary/grilling) and moving to implementation), `/compact` is the move. [ask-itech-skills](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/ask-itech-skills.md) carries the ordered tree over all five options at a phase boundary.
...
`handoff` is a **reach-for-it-anytime standalone** that lives at the seam between sessions rather than inside a build chain, but a narrow one, and the honest map is that you'll use it less often than the other four options at a phase boundary. Its closest neighbour is [prototype](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/prototype.md), because a prototype lives in its own directory and the round trip out and back is exactly the crossing this skill is for. When you're at a boundary and unsure whether to continue, clear, hand off, delegate or compact, [ask-itech-skills](https://github.com/rahularoratech/itech-skills/blob/main/docs/engineering/ask-itech-skills.md) carries the tree that orders those five, and routes you over the rest of the set.
